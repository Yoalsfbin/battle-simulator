...
function initCheatPanel(panelId = "cheatPanel") {
    const KONAMI_COMMAND = ["ArrowUp","ArrowUp","ArrowDown","ArrowDown","ArrowLeft","ArrowRight","ArrowLeft","ArrowRight","b","a"];
...
function cheatPanel(hero) {
    ...
    document.getElementById("spawnBoss").addEventListener("click", () => {
        // 既存の敵を全削除
        const enemies = document.querySelectorAll(".enemy1, .enemy2, .enemy3, .boss");
        ...
        displayText(document.querySelector(".boss").dataset.name + "があらわれた！", textContainer, showModal);
    });
}
